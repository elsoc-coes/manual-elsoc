...
El ponderador anterior luego es ajustado en base a la no respuesta inicial de la manzana en la que se encuentra cada observación. De este modo, damos más peso a las observaciones que están en manzanas donde pocas personas responden. 
...
La post-estratificación es un ajuste que suele hacerse en las encuestas para incorporar información conocida sobre la población en la estimación. 

En otras, palabras, es una manera de asegurar que la distribución poblacional en variables claves como totales regionales y por sexo coincidan en base a los totales poblacionales.

El ajuste de no respuesta puede pensarse como un factor que ajusta:

$$ps=N_j/\hat{N_j}$$

Los ponderadores de ELSOC tienen la forma:
$$w=w^{dis}*f_l*ps$$


Este ponderador luego es ajustado en base a la tasa de no respuesta inicial, y un ajuste de post-estratificación que busca luego con ajuste al número de casos ^[Corresponde a considerar la población total objetivo entre 18 y 75 años en base a las proyecciones de población
