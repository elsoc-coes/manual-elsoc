...
elsoc <- elsoc_long_2016_2022 %>%
  ...
  mutate(tipo_ola = ifelse(ola==1,"M. Original 2016",
        ...
        atricion =  ifelse(tipo_atricion==1,"2016-2021",
                    ifelse(tipo_atricion==2,"2016-2019",
                    ifelse(tipo_atricion==4,"2016-2018",
                    ifelse(tipo_atricion==8,"2016-2017",
                    ifelse(tipo_atricion==16,"2016",NA))))),
        atricion2 = ifelse(tipo_atricion==17,"2018-2021",
                    ifelse(tipo_atricion==18,"2018-2019",
                    ifelse(tipo_atricion==19,"2018",NA)))) %>%
...
```{r tabla-comparaciondemografica2021a}
fun_tabla_olas(5,0)  %>%
  kbl(align = c('c', 'c', 'c', 'c', 'c'),
      format = table_format,
      booktabs = TRUE) %>% 
...
```{r tabla-comparaciondemografica2021b}
fun_tabla_olas(3,1)  %>%
  kbl(align = c('c', 'c', 'c', 'c'),
      format = table_format,
      booktabs = TRUE) %>% 
...
```{r tabla-comparaciondemografica2021c}
fun_tabla_olas(4,1)  %>%
  kbl(align = c('c', 'c', 'c', 'c', 'c'),
      format = table_format,
      booktabs = TRUE) %>% 
...
```{r tabla-comparaciondemografica2021d}
fun_tabla_olas(5,1)  %>%
  kbl(align = c('c', 'c', 'c', 'c', 'c'),
      format = table_format,
      booktabs = TRUE) %>% 
...
```
### Comparación Sociodemográfica para la Atrición

Para la muestra original, se decide comparar las características sociodemográficas de las personas que responden sólo el 2016, sólo 2016-2017, sólo 2016-2017-2018, sólo 2016-2017-2018-2019 y quienes responden para todas las olas (2016-2021). Se observa que no hay grandes diferencias entre los distintos grupos que van dejando de responder la encuesta.

```{r atricion-sociodemografica-muestraoriginal}
tabla = (list(
  (elsoc %>% 
     filter(!is_nsnr(m0_sexo),ola==1,muestra==1) %>% 
     group_by(atricion) %>% 
     summarise("Hombre"=weighted.mean(m0_sexo_1,ponderador02))),
  (elsoc %>% 
     filter(!is_nsnr(m0_sexo),ola==1,muestra==1) %>% 
     ...
     summarise("Mujer"=weighted.mean(m0_sexo_2,ponderador02))),
  (elsoc %>% 
     filter(!is_nsnr(m0_edad),ola==1,muestra==1) %>% 
     group_by(atricion) %>% 
     summarise("18-29"=weighted.mean(etario_1,ponderador02))),
  (elsoc %>% 
     filter(!is_nsnr(m0_edad),ola==1,muestra==1) %>% 
  ...
  (elsoc %>% 
     ...
     summarise("65 o más"=weighted.mean(etario_4,ponderador02))),
  (elsoc %>% 
     filter(!is_nsnr(m01),ola==1,muestra==1) %>% 
     group_by(atricion) %>% 
     summarise("Sin estudios"=weighted.mean(m01_1,ponderador02))),
  (elsoc %>% 
     filter(!is_nsnr(m01),ola==1,muestra==1) %>% 
  ...
  reduce(merge,by="atricion") %>% 
  t() %>% 
  as.data.frame() %>%
  janitor::row_to_names(row_number = 1))[-6]
  
tabla = tabla %>% as.data.frame()
tabla[,1] = paste0(round(as.double(tabla[,1])*100,2),"%")
tabla[,2] = paste0(round(as.double(tabla[,2])*100,2),"%")
tabla[,3] = paste0(round(as.double(tabla[,3])*100,2),"%")
tabla[,4] = paste0(round(as.double(tabla[,4])*100,2),"%")
tabla[,5] = paste0(round(as.double(tabla[,5])*100,2),"%")

tabla %>% kbl(align = c('c', 'c', 'c', 'c','c'),
      format = table_format,
      booktabs = TRUE) %>% 
  kable_styling(full_width = FALSE,
                latex_options = 'HOLD_position') %>% 
...
```
Para la muestra refresco, se decide comparar las características sociodemográficas de las personas que responden sólo el 2018, sólo 2018-2019 y quienes responden para todas las olas (2018-2021). Se observa que no hay grandes diferencias entre los distintos grupos que van dejando de responder la encuesta.

```{r atricion-sociodemografica-muestrarefresco}
tabla = (list(
  (elsoc %>% 
     filter(!is_nsnr(m0_sexo),ola==3,muestra==2) %>% 
     group_by(atricion2) %>% 
     summarise("Hombre"=weighted.mean(m0_sexo_1,ponderador02))),
  (elsoc %>% 
     filter(!is_nsnr(m0_sexo),ola==3,muestra==2) %>% 
     ...
     summarise("Mujer"=weighted.mean(m0_sexo_2,ponderador02))),
  (elsoc %>% 
     filter(!is_nsnr(m0_edad),ola==3,muestra==2) %>% 
     group_by(atricion2) %>% 
     summarise("18-29"=weighted.mean(etario_1,ponderador02))),
  (elsoc %>% 
     filter(!is_nsnr(m0_edad),ola==3,muestra==2) %>% 
  ...
  (elsoc %>% 
     ...
     summarise("65 o más"=weighted.mean(etario_4,ponderador02))),
  (elsoc %>% 
     filter(!is_nsnr(m01),ola==3,muestra==2) %>% 
     group_by(atricion2) %>% 
     summarise("Sin estudios"=weighted.mean(m01_1,ponderador02))),
  (elsoc %>% 
     filter(!is_nsnr(m01),ola==3,muestra==2) %>% 
  ...
  (elsoc %>% 
    ...
    reduce(merge,by="atricion2") %>% 
    t() %>% 
    as.data.frame() %>%
    janitor::row_to_names(row_number = 1))[-4]
